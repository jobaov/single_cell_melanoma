...
We wish to identify doublets using the scDblFinder package, which gathers various methods for the detection and handling of doublets. We will use the computeDoubletDensity approach, where we obtain a doublet score for each cell based on the density of simulated doublets around it. Before doublet detection, we must normalize and find variable features according to the standard Seurat workflow. 
...
To visualize the data in an UMAP plot, linear dimensional reduction is performed.
...
Doublet scores are computed with computeDoubletDensity.

EPLAINATION FOR THIS. 
